...
export function PurchasesSection({ initialLines, onCreatedFromList, onPurchaseSaved }: PurchasesSectionProps) {
  ...
  function saveInlineProduct() {
    ...
    if (productForm.type === "packaged" && !productForm.pkgSize) return;
    const category = productForm.category || categories[0] || "Mercearia";
    ...
    const newItem: Item = {
      ...
      pkgUnit: productForm.type === "packaged" ? productForm.pkgUnit : undefined,
      alertDays: parseInt(productForm.alertDays) >= 0 ? parseInt(productForm.alertDays) : 15,
    };
    setItems([...items, newItem]);
    setLf({ ...lf, itemId: newItem.id, numPkgs: "", pkgQty: "", pricePerPkg: "", discount: "0" });
      ...
      <div className="space-y-4 pb-40">
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <span className={`text-[10px] font-bold ${isDark ? "text-slate-600" : "text-slate-400"}`}>{form.lines.length} adicionados</span>
          </div>
        ...
        {productModal && (
          <Modal title="Cadastrar Produto" onClose={() => setProductModal(false)}>
            <div className="space-y-4">
              ...
              {productForm.type === "packaged" && (
                <div className="grid grid-cols-2 gap-3">
                  <Inp inputRef={productPkgSizeRef} label="Qtd. na embalagem" type="number" value={productForm.pkgSize} onChange={v => setProductForm({ ...productForm, pkgSize: v })} placeholder="Ex: 80" min="0.001" step="0.001" required onEnter={saveInlineProduct} />

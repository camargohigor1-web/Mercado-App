...
// ─── Inline expanded card for market mode ─────────────────────────────────────
const MarketItemCard = memo(function MarketItemCard({
  ...
  // Keep localQty in sync when parent changes (e.g. +/- buttons)
  ...
  const lastPrice = stats
    ? item.type === "bulk" ? stats.lastPrice / factor : stats.lastPrice
    : null;
  const minPrice = stats
    ? item.type === "bulk" ? stats.minPrice / factor : stats.minPrice
    : null;
  const avgPrice = stats
    ? item.type === "bulk" ? stats.avgPrice / factor : stats.avgPrice
    : null;

  const priceVsAvg =
    lastPrice !== null && avgPrice !== null && avgPrice > 0
  ...
  return (
    ...
    >
      ...
      {isExpanded && (
        <div
          className={`border-t px-3 pb-3 pt-3 space-y-3 animate-fade-slide-up ${
        ...
        >
          ...
          {stats ? (
            <div className="grid grid-cols-3 gap-1.5">
              ...
              >
                <p
                  className={`text-[9px] font-black uppercase tracking-wide mb-1 ${
                    isDark ? "text-teal-700" : "text-teal-500"
                  }`}
                >
                  Mínimo
                </p>
                ...
                <p className={`text-[9px] ${isDark ? "text-teal-700" : "text-teal-500"}`}>
                  /{item.type === "bulk" ? du : "emb"}
                </p>
              </div>
              <div
                className={`rounded-xl p-2.5 ${
              ...
              >
                ...
                <p className={`text-[9px] ${isDark ? "text-green-700" : "text-green-500"}`}>
                  /{item.type === "bulk" ? du : "emb"}
                </p>
              </div>
              <div
                className={`rounded-xl p-2.5 ${
                  priceVsAvg !== null && priceVsAvg > 5
                ...
                <p
                  className={`text-[9px] ${
                    priceVsAvg !== null && priceVsAvg > 5
                      ...
                      : isDark ? "text-blue-700" : "text-blue-400"
                  }`}
                >
                  /{item.type === "bulk" ? du : "emb"}
                </p>
          ...
          {priceEvolution.length >= 2 && (
            ...
            >
              <LineChart
                data={priceEvolution}
                formatValue={fmt}
                unit={item.type === "bulk" ? `R$/${du}` : "R$/emb"}
              />
            </div>
          )}
            ...
            <div className="space-y-1.5">
              ...
              {recentEntries.map((e: any, i: number) => (
                <button
                  key={i}
                  onClick={() =>
                    onNavigatePurchase?.(e.purchaseId, itemId)
                  }
  ...
  return (
    <button
      ...
      className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl border transition-all active:scale-[0.98] text-left ${
        isDark
          ? "bg-slate-900/60 border-slate-800 hover:border-teal-500/40 hover:bg-teal-500/5"
      ...
      <div className="flex-1 min-w-0">
        ...
        {avgPrice !== null ? (
          <p className="text-[10px] text-green-400 mt-0.5">
            Médio {fmt(avgPrice)}/{item.type === "bulk" ? du : "emb"}
          </p>
        ) : (
...
export function ShoppingListSection({
  onConvertToPurchase,
  onGoToItems,
  onGoToHistoryPurchase,
  onGoToHistoryPurchaseWithProduct,
  ...
  const activeList = useMemo(
    () => shoppingList.filter((l): l is ShoppingListItem => !l.saved),
    [shoppingList]
  );
  const savedLists = useMemo(
    () => shoppingList.filter((l): l is SavedShoppingList => l.saved),
    [shoppingList]
  );
  const inListSet = useMemo(() => new Set(activeList.map((l) => l.itemId)), [activeList]);

  // stats cache — computed once
  const statsCache = useMemo(() => {
    const cache: Record<string, ReturnType<typeof calcStats>> = {};
    items.forEach((item) => {
      cache[item.id] = calcStats(
        item.id, items, purchases,
        warehouse.find((w) => w.itemId === item.id)?.entries ?? []
      );
    });
    return cache;
  }, [items, purchases, warehouse]);

  // recent entries cache per item
  ...
  // price evolution cache per item
  const priceEvolutionCache = useMemo(() => {
    const cache: Record<string, LineChartPoint[]> = {};
    items.forEach((item) => {
      const factor = getDisplayFactor(item);
      const du = getDisplayUnit(item);
      const entries = (recentEntriesCache[item.id] || [])
        .slice()
        .sort((a: any, b: any) => a.date.localeCompare(b.date));
      cache[item.id] = entries.map((e: any) => ({
        label: new Date(e.date + "T12:00:00").toLocaleDateString("pt-BR", {
          month: "short", year: "2-digit",
        }),
        value: item.type === "bulk"
          ? (e.pricePerUnit || 0) / factor
          : e.pricePerPkgAfterDiscount ?? e.pricePerPkg,
        date: e.date,
        market: e.market,
        qty: item.type === "bulk"
    ...
    () =>
      activeList
        .map((l) => ({
          ...
          stats: statsCache[l.itemId] ?? null,
          qty: (l as any).qty || 1,
        }))
        .filter((l) => l.item),
  ...
  // items not in list (for plan + quick-add)
  const availableItems = useMemo(
    () => items.filter((i) => !inListSet.has(i.id)),
    [items, inListSet]
  );
  ...
  const planFiltered = useMemo(() => {
    ...
    return availableItems
      ...
      .filter((i) => !filterCatAdd || (i.category || "Sem categoria") === filterCatAdd);
  }, [availableItems, planSearch, filterCatAdd]);
  ...
  // quick-add filtered (items not yet in list)
  ...
  // grouped market list by category
  ...
  // ── Save list with duplicate detection ────────────────────────────────────
  function attemptSave() {
    if (!saveName.trim() || activeList.length === 0) return;
    const existing = savedLists.find(
      (l) => l.name.toLowerCase() === saveName.trim().toLowerCase()
    );
    if (existing) {
      setDupModal({ name: saveName.trim(), existingId: existing.id });
      setSaveModal(false);
    } else {
      commitSaveAsNew();
    }
  ...
  function deleteSavedList(id: string) {
    setShoppingList(
      shoppingList.filter((l) => !(l.saved && (l as SavedShoppingList).id === id))
    );
  }
    ...
    const lines: PurchaseLine[] = activeList
      .map((li) => {
        ...
        if (item.type === "bulk") {
          ...
          return {
            itemId: li.itemId, numPkgs: qty, pkgQty,
            totalQty: qty * pkgQty, pricePerPkg,
            pricePerPkgAfterDiscount: pricePerPkg,
            discountTotal: 0, discountPerPkg: 0, pricePerUnit,
            total: pricePerPkg * qty,
          } as PurchaseLine;
        } else {
          const pricePerPkg = last.pricePerPkg || 0;
          return {
            itemId: li.itemId, numPkgs: qty, pricePerPkg,
            pricePerPkgAfterDiscount: pricePerPkg,
            discountTotal: 0, discountPerPkg: 0,
            pricePerInternal: last.pricePerInternal || 0,
            total: pricePerPkg * qty,
          } as PurchaseLine;
        }
      })
  ...
  function addCompareOption() {
    if (!newOption.size || !newOption.price || !priceCompareItem) return;
    const size = Number(newOption.size.replace(",", "."));
    const price = Number(newOption.price.replace(",", "."));
    if (size > 0 && price >= 0) {
      setCompareOptions((prev) => [
        ...prev,
        { sizeNum: size, priceNum: price, unit: getDisplayUnit(priceCompareItem) },
      ]);
  ...
  const bestOption = useMemo(() => {
    ...
    return compareOptions
      .map((o) => ({ ...o, ppu: o.priceNum / o.sizeNum }))
      .reduce((a, b) => (b.ppu < a.ppu ? b : a));
  }, [compareOptions]);

  // ── Navigate to purchase ───────────────────────────────────────────────────
  function handleNavigatePurchase(purchaseId: string, itemId: string) {
    if (onGoToHistoryPurchaseWithProduct) {
      onGoToHistoryPurchaseWithProduct(purchaseId, itemId);
    } else if (onGoToHistoryPurchase) {
      onGoToHistoryPurchase(purchaseId);
    }
  }
    ...
    <div className="space-y-4">
        ...
        <>
          ...
          <div className="flex items-center justify-between gap-2 flex-wrap">
            <div className="flex gap-2">
              {availableItems.some((i) => (statsCache[i.id]?.avgMonthly ?? 0) > 0) && (
                <Btn onClick={autoSuggest} variant="outline" size="sm">
                  <Icon name="history" size={13} />Sugerir
                </Btn>
              )}
              {savedLists.length > 0 && (
            ...
            <div className="flex gap-2">
              {activeList.length > 0 && (
                <>
                  <Btn onClick={() => setClearAllConfirm(true)} variant="danger" size="sm">
                    <Icon name="trash" size={13} />
                  </Btn>
                  <Btn onClick={() => setShareModal(true)} variant="outline" size="sm">
                    <Icon name="share" size={13} />
                  </Btn>
                  <Btn onClick={() => setConvertModal(true)} variant="outline" size="sm">
                    <Icon name="cart" size={13} />Compra
                  </Btn>
          ...
          {listFull.length > 0 && (
            <div>
              ...
              <div className="space-y-1.5">
                {listFull.map(({ itemId, item, stats, qty, done }) => {
                  const it = item!;
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {avgPrice !== null && !done && (
                          <p className="text-[10px] text-green-400">
                            Médio {fmt(avgPrice)}/{it.type === "bulk" ? du : "emb"}
                          </p>
                        )}
                      </div>
                      <div className="flex items-center gap-1 flex-shrink-0">
          ...
          <div>
            ...
            {planCategories.length > 1 && (
              <select value={filterCatAdd} onChange={(e) => setFilterCatAdd(e.target.value)}
                className={`w-full mb-2 ${isDark ? "bg-slate-900 border-slate-700 text-slate-100" : "bg-white border-slate-300 text-slate-900"} border rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-teal-500 appearance-none`}>
                <option value="">Todas as categorias</option>
                {planCategories.map((cat) => <option key={cat} value={cat}>{cat}</option>)}
              </select>
            )}
            <input value={planSearch} onChange={(e) => setPlanSearch(e.target.value)}
              placeholder="Buscar produto..."
              className={`w-full mb-3 ${isDark ? "bg-slate-900 border-slate-700 text-slate-100 placeholder-slate-700" : "bg-white border-slate-300 text-slate-900 placeholder-slate-400"} border rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-teal-500 transition-all`} />
            ...
            {items.length === 0 ? (
              ...
              <p className={`text-xs text-center py-5 ${isDark ? "text-slate-600" : "text-slate-400"}`}>
                Todos os produtos já estão na lista
              </p>
            ) : planFiltered.length === 0 ? (
              <p className={`text-xs text-center py-5 ${isDark ? "text-slate-600" : "text-slate-400"}`}>
                Nenhum resultado para "{planSearch}"
              </p>
            ) : (
              <div className="space-y-4">
                        ...
                        return (
                          <button key={item.id} onClick={() => addItem(item.id)}
                            className={`w-full text-left px-3 py-2.5 rounded-xl border flex items-center justify-between gap-3 transition-all active:scale-[0.98] ${
                              isDark ? "bg-slate-900 border-slate-800 hover:border-teal-500/50 hover:bg-teal-500/5" : "bg-white border-slate-200 hover:border-teal-400 hover:bg-teal-50/50"
                            }`}>
            ...
            <div className="flex flex-col items-center text-center py-12 gap-4 px-4">
              ...
              <div className="space-y-1">
                ...
                <p className={`text-xs ${isDark ? "text-slate-500" : "text-slate-400"}`}>
                  Vá para "Planejar" e adicione produtos antes de ir ao mercado.
                </p>
              </div>
              <Btn onClick={() => setListMode("plan")} size="sm">
                <Icon name="plus" size={13} />Planejar compras
              </Btn>
            </div>
          ) : (
            <>
              ...
              <div className={`flex items-center justify-between px-4 py-3 rounded-xl border ${isDark ? "bg-slate-900 border-slate-800" : "bg-slate-50 border-slate-200"}`}>
                <div>
                  ...
                  <p className={`text-sm font-bold mt-0.5 ${isDark ? "text-slate-100" : "text-slate-900"}`}>
                    <span className={pendingCount > 0 ? "" : "text-teal-400"}>{pendingCount} pendente{pendingCount !== 1 ? "s" : ""}</span>
                    {doneCount > 0 && <span className="text-slate-500"> · <span className="text-teal-400">{doneCount} comprado{doneCount !== 1 ? "s" : ""}</span></span>}
                  </p>
                </div>
                <div className="flex gap-2">
                  {doneCount > 0 && (
                    <Btn onClick={clearDone} variant="ghost" size="sm">Limpar</Btn>
                  )}
                  <Btn onClick={() => setConvertModal(true)} variant="outline" size="sm">
                ...
                <div className="flex gap-2 overflow-x-auto pb-0.5 scrollbar-hide">
                  {["", ...marketCategories].map((cat) => (
                    <button key={cat || "__all__"}
                      onClick={() => setFilterCatMarket(cat)}
                      ...
                      <div className="space-y-2">
                        {catItems.map(({ itemId, item, stats, qty }) => (
                          <MarketItemCard
                            ...
                            onNavigatePurchase={
                              (onGoToHistoryPurchase || onGoToHistoryPurchaseWithProduct)
                                ? handleNavigatePurchase
                                : undefined
                            }
                          />
                        ))}
                ...
                <div className="space-y-1.5">
                  ...
                  {doneList.map(({ itemId, item, qty }) => (
                    <div key={itemId} className={`flex items-center gap-3 px-3 py-2.5 rounded-xl border opacity-50 ${isDark ? "bg-slate-900 border-slate-800" : "bg-slate-50 border-slate-200"}`}>
                      <button onClick={() => toggleDone(itemId)}
                        className="w-6 h-6 rounded-full bg-teal-500 border-2 border-teal-500 flex-shrink-0 flex items-center justify-center">
                      ...
                      <button onClick={() => removeItem(itemId)} className={`p-1 ${isDark ? "text-slate-700" : "text-slate-400"}`}>
                        <Icon name="x" size={13} />
                      </button>
                    </div>
                  ))}
                  ...
                  <button
                    ...
                    className={`w-full flex items-center justify-center gap-2 py-3 rounded-xl border-2 border-dashed text-sm font-bold transition-all ${
                      showQuickAdd
                        ...
                        : isDark ? "border-slate-700 text-slate-500 hover:border-slate-600 hover:text-slate-300" : "border-slate-300 text-slate-400 hover:border-slate-400 hover:text-slate-600"
                    }`}
                  >
                    <Icon name={showQuickAdd ? "x" : "plus"} size={16} />
                    {showQuickAdd ? "Fechar" : `Adicionar item à lista (${availableItems.length} disponíveis)`}
                  </button>
                )}

                {showQuickAdd && (
                  <div className={`mt-3 rounded-2xl border overflow-hidden ${isDark ? "bg-slate-900 border-slate-800" : "bg-white border-slate-200"}`}>
                    {/* Search inside quick add */}
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          className={`w-full pl-8 pr-3 py-2 rounded-xl text-sm focus:outline-none focus:border-teal-500 border transition-all ${
                            isDark ? "bg-slate-800 border-slate-700 text-slate-100 placeholder-slate-600" : "bg-slate-50 border-slate-200 text-slate-900 placeholder-slate-400"
                          }`}
                        />
                        {quickSearch && (
                    ...
                    <div className="max-h-64 overflow-y-auto">
                      {quickAddFiltered.length === 0 ? (
                        <div className="py-6 text-center">
                          ...
                          {onGoToItems && (
                            <button onClick={onGoToItems}
                              className="mt-2 text-xs text-teal-400 font-bold flex items-center gap-1 mx-auto">
                        ...
                        <div className="p-2 space-y-1">
                          {quickAddFiltered.map((item) => (
                            <QuickAddItem
                              key={item.id}
                              item={item}
                              stats={statsCache[item.id] ?? null}
                              isDark={isDark}
                              onAdd={() => {
                                addItem(item.id);
                                // if list is now single item, briefly show it
                                if (quickAddFiltered.length === 1) setShowQuickAdd(false);
                              }}
                            />
                          ))}
                        </div>
      ...
      {saveModal && (
        <Modal title="Salvar Lista" onClose={() => setSaveModal(false)}>
          <div className="space-y-4">
            <Inp label="Nome da lista" value={saveName} onChange={setSaveName}
              placeholder="Ex: Compras do mês…" required onEnter={attemptSave} />
            {savedLists.length > 0 && (
              <div className={`rounded-xl p-3 ${isDark ? "bg-slate-900/50" : "bg-slate-50"}`}>
                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">
                  Ou atualizar uma lista existente
                </p>
                <div className="space-y-1.5">
                  {savedLists.map((sl) => (
                    <button key={sl.id} onClick={() => { updateSavedListDirect(sl.id); setSaveModal(false); }}
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-slate-500">
                          {sl.items.length} itens · {new Date(sl.date + "T12:00:00").toLocaleDateString("pt-BR")}
                        </p>
                      </div>
                      <span className={`text-[10px] font-bold px-2 py-1 rounded-lg ${isDark ? "bg-blue-500/15 text-blue-400" : "bg-blue-50 text-blue-600"}`}>
                        Atualizar
                      </span>
                    </button>
                  ))}
      ...
      {dupModal && (
        <Modal title="Lista já existe" onClose={() => { setDupModal(null); setSaveModal(true); }}>
          <div className="space-y-4">
            ...
            <Btn onClick={() => { setDupModal(null); setSaveModal(true); }} variant="secondary" className="w-full justify-center">
              Cancelar
            </Btn>
          </div>
        </Modal>
              ...
              <Card key={list.id}>
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex gap-1 flex-shrink-0">
                    <Btn onClick={() => loadSavedList(list)} size="sm" variant="success">
                      <Icon name="copy" size={12} />Carregar
                    </Btn>
                    <button onClick={() => deleteSavedList(list.id)}
                      className={`p-1.5 ${isDark ? "text-slate-600 hover:text-red-400" : "text-slate-400 hover:text-red-500"}`}>
                      <Icon name="trash" size={13} />
                    </button>
                  </div>
                </div>
        ...
        <Modal title="Converter em Compra" onClose={() => setConvertModal(false)}>
          <div className="space-y-4">
            ...
            <div className="flex gap-3">
              ...
              <Btn onClick={handleConvertToPurchase} className="flex-1">
                <Icon name="cart" size={15} />Criar Compra
              </Btn>
            </div>
          </div>
        </Modal>
      ...
      {clearAllConfirm && (
        <ConfirmModal title="Limpar lista inteira"
          message="Remover todos os itens da lista ativa? Esta ação não pode ser desfeita."
          ...
          <div className="space-y-4">
                ...
                return Object.entries(g).sort(([a], [b]) => a.localeCompare(b)).map(([cat, catItems]) => (
                  <div key={cat} className="mb-2">
                    ...
                    {catItems.map((li) => {
                      ...
                      if (!item) return null;
                      const qty = (li as any).qty || 1;
                      return <p key={li.itemId} className="ml-2">☐ {qty > 1 ? `${qty}x ` : ""}{item.name}</p>;
                    })}
                  </div>
                ));
              })()}
            ...
            <div className="space-y-2">
              ...
              <button onClick={() => {
                ...
                if (navigator.share) navigator.share({ title: "Lista de Compras", text }).catch(() => {});
      ...
      {/* Price compare modal */}
      {priceCompareItem && (
        <Modal title={`Comparar — ${priceCompareItem.name}`} onClose={() => setPriceCompareItem(null)}>
          <div className="space-y-4">
            ...
            <div className={`space-y-3 ${isDark ? "bg-slate-900/50" : "bg-slate-50"} rounded-xl p-3`}>
              ...
              <div className="grid grid-cols-2 gap-2">
                <Inp inputRef={cmpSizeRef} label={`Quantidade (${getDisplayUnit(priceCompareItem)})`} type="number" value={newOption.size}
            ...
            <Btn onClick={() => setPriceCompareItem(null)} variant="secondary" className="w-full justify-center">Fechar</Btn>
          </div>
        </Modal>
      )}
    </div>

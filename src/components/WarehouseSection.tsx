...
export function WarehouseSection({ items, purchases, markets, warehouse, setWarehouse, categories, shoppingList, setShoppingList, onGoToNewPurchase, onSelectionChange, initialSearch }: WarehouseSectionProps) {
  ...
  if (sortPrimary === "category") {
    categories.forEach(cat => {
      const g = sortedItems.filter(({ item }) => item.category === cat);
  ...
  function doUndo() {
    ...
    setUndoSnapshot(null); setZeroFeedback(null);
    ...
    return (
      <Card key={item.id} onClick={() => setSelectedId(item.id)} className={`${isDark ? "hover:border-slate-600" : "hover:border-slate-300"} ${isChecked ? (isDark ? "border-amber-500/40 bg-amber-500/5" : "border-amber-300 bg-amber-50/50") : ""}`}>
        <div className="flex items-center gap-3">
          ...
          <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${isLow ? "bg-amber-500/15 text-amber-400" : "bg-teal-500/20 text-teal-400"}`}>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              {sortPrimary !== "category" && <span className={`text-[9px] ${isDark ? "text-slate-600" : "text-slate-400"}`}>{item.category}</span>}
              {isLow && <span className={`text-[9px] font-black px-1.5 py-0.5 rounded-md uppercase tracking-wide ${isDark ? "bg-amber-500/15 text-amber-400" : "bg-amber-100 text-amber-700"}`}>Baixo</span>}
            </div>
            <div className="flex gap-3 mt-0.5 flex-wrap">
              ...
              {avgMonthly > 0 && <span className="text-xs text-slate-500">Cons.: <span className={isDark ? "text-slate-300" : "text-slate-700"}>{fmtN(avgMonthly, item.type === "packaged" ? 1 : 2)}/mês</span></span>}
              {daysLeft !== null && <span className={`text-xs font-bold ${isLow ? "text-amber-400" : "text-slate-500"}`}>~{daysLeft}d</span>}
            </div>
          </div>
          <div className="flex items-center gap-1.5">
            <button onClick={e => { e.stopPropagation(); openUpdate(item.id); }} className={`p-1.5 ${isDark ? "text-slate-600 hover:text-teal-400" : "text-slate-400 hover:text-teal-500"} transition-colors`}>
              <Icon name="refresh" size={14} />
            </button>
            <span className={isDark ? "text-slate-600" : "text-slate-400"}><Icon name="chevron" size={16} /></span>
          </div>
        </div>
      </Card>
    );
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <div className="flex gap-1.5 mt-0.5 flex-wrap">
              ...
              {item.alertDays === 0 ? <Badge color="slate">sem alerta</Badge> : <Badge color="amber">alerta: {item.alertDays ?? 15}d</Badge>}
            </div>
          </div>
                ...
                <Card key={e.id}>
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-slate-500 mt-0.5">Anterior: {fmtN(e.previousStock * factor, item.type === "packaged" ? 0 : 2)} {du}</p>
                      {e.consumed > 0 && <p className="text-xs text-amber-400 mt-0.5">Consumo: {fmtN(e.consumed * factor, item.type === "packaged" ? 0 : 2)} {du}</p>}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className={`flex gap-2 ${isDark ? "bg-slate-900" : "bg-slate-100"} rounded-xl p-1`}>
        <button onClick={() => setView("current")} className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${view === "current" ? "bg-teal-500 text-white" : isDark ? "text-slate-500 hover:text-slate-300" : "text-slate-500 hover:text-slate-700"}`}>Atual</button>
        <button onClick={() => setView("alerts")} className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${view === "alerts" ? "bg-red-500 text-white" : isDark ? "text-slate-500 hover:text-slate-300" : "text-slate-500 hover:text-slate-700"}`}>
        ...
        lowStockItems.length === 0 ? (
          ...
          <div className="space-y-2">
            <div className={`flex items-center gap-3 ${isDark ? "bg-red-500/5 border-red-500/20" : "bg-red-50 border-red-200"} border rounded-xl px-4 py-3`}>
              <div className="w-8 h-8 rounded-xl bg-red-500/20 text-red-400 flex items-center justify-center flex-shrink-0"><Icon name="warn" size={14} /></div>
              <div>
                <p className="text-[10px] font-black text-red-400 uppercase tracking-widest">Estoque baixo</p>
                <p className="text-xs text-slate-500">{lowStockItems.length} produto{lowStockItems.length === 1 ? "" : "s"} abaixo do limite.</p>
              </div>
            </div>
              ...
              return (
                <Card key={item.id} onClick={() => setSelectedId(item.id)} className={isDark ? "hover:border-red-500/40" : "hover:border-red-300"}>
                  <div className="flex items-start gap-3">
                    <div className="w-9 h-9 rounded-xl bg-red-500/20 text-red-400 flex items-center justify-center flex-shrink-0"><Icon name="warn" size={15} /></div>
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex gap-3 mt-0.5 flex-wrap">
                        ...
                        <span className="text-xs text-slate-500">Limite: <span className="text-amber-400">{threshold}d</span></span>
                      </div>
                      <div className="flex gap-2 mt-3 flex-wrap">
                        <button onClick={e => { e.stopPropagation(); addToShoppingList(item.id); }} disabled={alreadyInList}
                          className={`inline-flex items-center gap-1.5 rounded-xl px-3 py-1.5 text-xs font-bold transition-all disabled:opacity-60 ${alreadyInList ? isDark ? "bg-slate-800 text-slate-400" : "bg-slate-200 text-slate-500" : "bg-teal-500/15 text-teal-400 hover:bg-teal-500/25 border border-teal-500/30"}`}>
                          <Icon name={alreadyInList ? "check" : "plus"} size={12} />{alreadyInList ? "Na lista" : "Adicionar à lista"}
                        </button>
                        <button onClick={e => { e.stopPropagation(); openUpdate(item.id); }}
      ...
      {view === "current" && (warehouseItems.length === 0 ? (
        <div className="flex flex-col items-center text-center py-12 gap-4 px-4">
          <div className={`w-14 h-14 rounded-2xl flex items-center justify-center ${isDark ? "bg-slate-800" : "bg-slate-100"}`}><Icon name="warehouse" size={24} /></div>
          <div className="space-y-1">
            <p className={`font-black text-sm ${isDark ? "text-slate-200" : "text-slate-800"}`}>Armazém vazio</p>
            <p className={`text-xs ${isDark ? "text-slate-500" : "text-slate-400"}`}>O estoque é atualizado automaticamente quando você registra compras.</p>
          </div>
          {onGoToNewPurchase && (
            <button onClick={onGoToNewPurchase} className="px-5 py-2.5 rounded-xl bg-teal-500 text-white text-xs font-black shadow-lg shadow-teal-500/25 active:scale-95 transition-transform">
              Registrar compra
            </button>
          )}
        </div>
              ...
              return (
                <div key={cat}>
                  <div className="flex items-center justify-between mb-2 px-1">
                    <p className={`text-[10px] font-black uppercase tracking-widest ${isDark ? "text-slate-600" : "text-slate-400"}`}>{cat}</p>
                    <button onClick={toggleCat} className={`text-[10px] font-bold transition-all ${allSelected ? isDark ? "text-amber-400" : "text-amber-600" : isDark ? "text-slate-600 hover:text-slate-400" : "text-slate-400 hover:text-slate-600"}`}>
      ...
      {showZeroModal && (
        <Modal title="Zerar estoque" onClose={() => setShowZeroModal(false)}>
          <div className="space-y-4">
            <p className={`text-sm ${isDark ? "text-slate-400" : "text-slate-600"}`}>Confirme os itens que terão o estoque zerado:</p>
                  ...
                  <Card key={itemId} className="py-2">
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-slate-500">Estoque: <span className="text-teal-400 font-bold">{fmtN(stock, item.type === "packaged" ? 0 : 2)} {du}</span></p>
                      </div>
                      <button onClick={() => removeFromZeroList(itemId)} className={`p-1.5 ${isDark ? "text-slate-600 hover:text-red-400" : "text-slate-400 hover:text-red-500"} flex-shrink-0`}><Icon name="x" size={13} /></button>
                    </div>
                  </Card>
                );
              })}
            </div>
            {selectedIds.size === 0 && <p className={`text-xs text-center py-2 ${isDark ? "text-slate-500" : "text-slate-400"}`}>Nenhum item restante</p>}
            {selectedIds.size > 0 && (
      ...
      {zeroFeedback && (
        <div className="fixed bottom-28 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 px-4 py-2.5 rounded-xl bg-slate-800 text-white text-xs font-bold shadow-xl border border-slate-700 animate-fade-slide-up whitespace-nowrap">
          ...
          {undoSnapshot && (
            <button onClick={doUndo} className="text-teal-400 font-black hover:text-teal-300 border-l border-slate-700 pl-3 ml-1">Desfazer</button>
          )}
        </div>
      )}
            ...
            <div className="space-y-4">
              ...
              {updateForm.qty !== "" && +updateForm.qty >= 0 && (
                <div className={`${isDark ? "bg-slate-800/60" : "bg-slate-100"} rounded-xl p-3`}>
                  <p className="text-xs text-slate-500 mb-1">Consumo a registrar</p>
